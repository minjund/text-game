    ...
    <div v-if="show" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm"
      ...
      <div class="bg-slate-900/95 rounded-2xl border-2 border-purple-500/50 shadow-2xl max-w-6xl w-full max-h-[90vh] overflow-hidden">
        ...
        <div class="p-6 overflow-y-auto max-h-[calc(90vh-180px)]">
          <!-- 시너지 효과 설명 -->
          <div class="mb-8 bg-gradient-to-br from-purple-900/30 to-blue-900/30 rounded-xl border-2 border-purple-500/30 p-6">
          ...
          <div class="mb-8">
            <div class="flex items-center gap-3 mb-4">
              <h3 class="text-2xl font-bold text-yellow-400">💰 무역 카드</h3>
              <span class="px-3 py-1 rounded-full text-sm font-bold bg-yellow-600 text-yellow-100">
                3개 시너지
              </span>
            </div>

            <!-- 시너지 효과 -->
            <div class="mb-4 bg-gradient-to-br from-yellow-900/30 to-orange-900/30 rounded-xl border-2 border-yellow-500/30 p-4">
              <div class="flex items-start gap-4">
                <div class="text-5xl">👑</div>
                <div class="flex-1">
                  <h4 class="text-xl font-bold text-yellow-300 mb-2">무역 제국 (3개 시너지)</h4>
                  <p class="text-sm text-slate-300 mb-3">무역 카드 3개를 모으면 발동</p>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div v-for="card in tradeCards" :key="card.id"
                   class="bg-gradient-to-br from-slate-800 to-slate-900 rounded-xl border-2 border-yellow-600/50 p-4">
                <div class="text-4xl text-center mb-3">{{ card.icon }}</div>
                <h4 class="text-lg font-bold text-center mb-2 text-yellow-300">{{ card.name }}</h4>
                <p class="text-xs text-center text-slate-300 mb-3">{{ card.description }}</p>
                <div class="space-y-1 bg-slate-800/50 rounded-lg p-3">
          ...
          <div class="mb-8">
            <div class="flex items-center gap-3 mb-4">
              <h3 class="text-2xl font-bold text-red-400">⚔️ 군사 카드</h3>
              <span class="px-3 py-1 rounded-full text-sm font-bold bg-red-600 text-red-100">
                3개 시너지
              </span>
            </div>

            <!-- 시너지 효과 -->
            <div class="mb-4 bg-gradient-to-br from-red-900/30 to-orange-900/30 rounded-xl border-2 border-red-500/30 p-4">
              <div class="flex items-start gap-4">
                <div class="text-5xl">🏆</div>
                <div class="flex-1">
                  <h4 class="text-xl font-bold text-red-300 mb-2">무적의 군단 (3개 시너지)</h4>
                  <p class="text-sm text-slate-300 mb-3">군사 카드 3개를 모으면 발동</p>
                  <div class="flex gap-4">
                    <div class="flex items-center gap-2 text-sm">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div v-for="card in militaryCards" :key="card.id"
                   class="bg-gradient-to-br from-slate-800 to-slate-900 rounded-xl border-2 border-red-600/50 p-4">
                <div class="text-4xl text-center mb-3">{{ card.icon }}</div>
                <h4 class="text-lg font-bold text-center mb-2 text-red-300">{{ card.name }}</h4>
                <p class="text-xs text-center text-slate-300 mb-3">{{ card.description }}</p>
                <div class="space-y-1 bg-slate-800/50 rounded-lg p-3">
          ...
          <div class="mb-8">
            <div class="flex items-center gap-3 mb-4">
              <h3 class="text-2xl font-bold text-green-400">🌾 농업 카드</h3>
              <span class="px-3 py-1 rounded-full text-sm font-bold bg-green-600 text-green-100">
                2개 시너지
              </span>
            </div>

            <!-- 시너지 효과 -->
            <div class="mb-4 bg-gradient-to-br from-green-900/30 to-emerald-900/30 rounded-xl border-2 border-green-500/30 p-4">
              <div class="flex items-start gap-4">
                <div class="text-5xl">🌾</div>
                <div class="flex-1">
                  <h4 class="text-xl font-bold text-green-300 mb-2">풍년의 축복 (2개 시너지)</h4>
                  <p class="text-sm text-slate-300 mb-3">농업 카드 2개를 모으면 발동</p>
                  <div class="flex gap-4">
                    <div class="flex items-center gap-2 text-sm">
                      <span class="text-green-400">🌾</span>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div v-for="card in agricultureCards" :key="card.id"
                   class="bg-gradient-to-br from-slate-800 to-slate-900 rounded-xl border-2 border-green-600/50 p-4">
                <div class="text-4xl text-center mb-3">{{ card.icon }}</div>
                <h4 class="text-lg font-bold text-center mb-2 text-green-300">{{ card.name }}</h4>
                <p class="text-xs text-center text-slate-300 mb-3">{{ card.description }}</p>
                <div class="space-y-1 bg-slate-800/50 rounded-lg p-3">
                  <div v-if="card.effect.food" class="flex items-center justify-between text-sm">
                    <span class="text-green-400">🌾</span>
          ...
          <div class="mb-8">
            <div class="flex items-center gap-3 mb-4">
              <h3 class="text-2xl font-bold text-purple-400">🎭 문화 카드</h3>
              <span class="px-3 py-1 rounded-full text-sm font-bold bg-purple-600 text-purple-100">
                2개 시너지
              </span>
            </div>

            <!-- 시너지 효과 -->
            <div class="mb-4 bg-gradient-to-br from-purple-900/30 to-pink-900/30 rounded-xl border-2 border-purple-500/30 p-4">
              <div class="flex items-start gap-4">
                <div class="text-5xl">✨</div>
                <div class="flex-1">
                  <h4 class="text-xl font-bold text-purple-300 mb-2">문화 부흥 (2개 시너지)</h4>
                  <p class="text-sm text-slate-300 mb-3">문화 카드 2개를 모으면 발동</p>
                  <div class="flex gap-4">
                    <div class="flex items-center gap-2 text-sm">
                      <span class="text-pink-400">❤️</span>
                      <span class="text-white font-bold">민심 +3</span>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div v-for="card in cultureCards" :key="card.id"
                   class="bg-gradient-to-br from-slate-800 to-slate-900 rounded-xl border-2 border-purple-600/50 p-4">
                <div class="text-4xl text-center mb-3">{{ card.icon }}</div>
                <h4 class="text-lg font-bold text-center mb-2 text-purple-300">{{ card.name }}</h4>
                <p class="text-xs text-center text-slate-300 mb-3">{{ card.description }}</p>
                <div class="space-y-1 bg-slate-800/50 rounded-lg p-3">
